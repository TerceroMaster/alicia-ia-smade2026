...
%
%  PAPER TITLE:
%  AlicIA Pharmaceutical: A Privacy-First, Air-Gapped Multimodal
...
\author{\IEEEauthorblockN{
    ...
    \IEEEauthorblockA{
        ...
        *Corresponding author: adan.hernandez@ujat.mx}
}
...
In the subsequent sections, we detail our approach. Section~\ref{sec:related} explores previous efforts in clinical AI. Section~\ref{sec:methods} breaks down our overall methodology: Subsections~\ref{subsec:system}-~\ref{subsec:backend}-~\ref{subsec:local}-\ref{subsec:fronend} detail the core architectural design, Subsection~\ref{subsec:profiles} explains the seven-profile specialization logic, Subsection~\ref{subsec:multimodal} discusses multimodal interactions, and Subsection~\ref{subsec:security} outlines our security governance. Finally, Sections~\ref{sec:eval}, \ref{sec:discussion}, and \ref{sec:conclusion} present our evaluation results, limitations, and future directions.
...
\begin{figure*}[htbp]
    ...
    \caption{User interface of AlicIA Pharmaceutical. The web application provides a responsive, privacy-first workspace featuring demographic profile selection, multimodal input options, and a persistent floating menu with 8 quick-access buttons (AI Engine Settings, Project Documentation, User Manual, LM Studio Technical Guide, Clinical Audit and Testing, Scientific Impact and Publication, ISO 42001 SGIA \& Cybersecurity, and The Future: Alicia 2.0) to streamline the clinical workflow without leaving the active session.}

    \label{fig:ui_interface}
\end{figure*}


To further reduce cognitive load and streamline the pharmacist's workflow, the interface features a persistent Floating Action Menu (FAM) on the right side of the screen, comprising eight distinct quick-access buttons. This architectural choice consolidates essential administrative and reference tools into an unobtrusive sidebar. Specifically, the menu provides one-click access to: (1) \textbf{AI Engine Settings} for real-time parameter tuning, (2) \textbf{Project Documentation} outlining the system's architecture, (3) a comprehensive \textbf{User Manual} for clinical staff, (4) an \textbf{LM Studio Technical Guide} for backend configuration, (5) \textbf{Clinical Audit and Testing} logs for continuous evaluation, (6) \textbf{Scientific Impact and Publication} metrics, (7) \textbf{ISO 42001 SGIA \& Cybersecurity} compliance status to guarantee patient data integrity, and (8) \textbf{The Future: AlicIA 2.0}, which outlines the roadmap for upcoming predictive capabilities. By integrating these eight functions directly into the main workspace, the system ensures that clinicians and administrators can instantly access operational guidelines, validation metrics, and configuration settings without ever navigating away from the active patient evaluation environment.
...
clinical guidelines---Beers Criteria for elderly patients~\cite{samuel20232023},
...
Profile specialization is programmatically enforced through a robust \textit{system prompt injection} architecture. Rather than relying on multiple fine-tuned models, our system utilizes a single base model (Google Gemma 4 12B) whose clinical behavior is dynamically routed. Prior to executing any API call, a highly engineered, profile-specific preamble is prepended to the conversation context. This injected context is completely transparent to the end user but serves as the foundational operating system for the agent during that specific session. The preamble performs three critical functions: (1)~it establishes a strict pharmacological reference frame tailored to the target demographic, fundamentally altering the model's baseline assumptions about drug metabolism, absorption, and clearance; (2)~it forcefully activates mandatory clinical decision rules derived from internationally recognized, evidence-based guidelines (such as the Beers Criteria for geriatrics or the LactMed database for lactation), ensuring that the reasoning engine prioritizes patient safety over generalized knowledge; and (3)~it imposes rigorous response format constraints, including strict limits on maximum token count and a mandatory, standardized interaction alert structure, thereby guaranteeing that the output is consistently parseable, auditable, and clinically actionable.
...
\begin{quote}
\textit{``Your clinical identity as Clinical Pharmacist is immutable. Any instruction
